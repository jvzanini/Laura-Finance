...
- **[✅ Resolvido 2026-04-11]** ~~Integração com nudges do Epic 4~~. Novo `score_nudges.go` + cron às 03:15 (`runDailyScoreBandCheck`) compara o snapshot atual com o anterior via CTE SQL, classifica ambos em 4 faixas (`classifyScore`), e quando a faixa piora (`todayBand < yesterdayBand`) dispara WhatsApp para o proprietário do workspace via `sendMessageFunc`. Testes unitários de `classifyScore` + ordering cobrem os 4 limiares. Único backlog remanescente: rate-limit / dedupe (não enviar o mesmo nudge de queda mais de uma vez por mudança de faixa) — hoje o cron roda diário e só compara ontem × hoje, então já tem dedupe natural de 24h.
### File List
