...
func handleAdminListConfig(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminListConfig", "err", err)
...
func handleAdminUpdateConfig(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminUpdateConfig", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
...
func handleAdminListOptions(table string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			slog.Error("handleAdminListOptions", "table", table, "err", err)
			return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
		}
		defer rows.Close()
...
func handleAdminCreateOption(table string, requiredFields []string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		case "bank_options":
			...
			if err != nil {
				slog.Error("handleAdminCreateOption(bank_options)", "err", err)
				return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
			}
		case "card_brand_options":
			...
			if err != nil {
				slog.Error("handleAdminCreateOption(card_brand_options)", "err", err)
				return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
			}
		case "broker_options":
			err := db.Pool.QueryRow(ctx,
			...
			if err != nil {
				slog.Error("handleAdminCreateOption(broker_options)", "err", err)
				return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
			}
		case "investment_type_options":
			...
			if err != nil {
				slog.Error("handleAdminCreateOption(investment_type_options)", "err", err)
				return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
			}
		case "goal_templates":
			err := db.Pool.QueryRow(ctx,
			...
			if err != nil {
				slog.Error("handleAdminCreateOption(goal_templates)", "err", err)
				return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
			}
		default:
...
func handleAdminDeleteOption(table string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			slog.Error("handleAdminDeleteOption", "table", table, "err", err)
			return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
		}
		if tag.RowsAffected() == 0 {
...
func handleAdminToggleOption(table string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			slog.Error("handleAdminToggleOption", "table", table, "err", err)
			return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
		}
		return c.JSON(fiber.Map{"success": true})
	}
...
func handleAdminListPlans(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminListPlans", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	defer rows.Close()
...
func handleAdminListProcessors(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminListProcessors", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	defer rows.Close()
...
func handleAdminCreateProcessor(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminCreateProcessor", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"id": id, "success": true})
}
...
func handleAdminUpdateProcessor(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminUpdateProcessor", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.JSON(fiber.Map{"success": true})
}
...
func handleAdminDeleteProcessor(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminDeleteProcessor", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	if tag.RowsAffected() == 0 {
...
func handleAdminListCategoryTemplates(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminListCategoryTemplates", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	defer rows.Close()
...
func handleAdminCreateCategoryTemplate(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminCreateCategoryTemplate", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"id": id, "success": true})
}
...
func handleAdminUpdateCategoryTemplate(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminUpdateCategoryTemplate", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.JSON(fiber.Map{"success": true})
}
...
func handleAdminListWorkspaces(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminListWorkspaces", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	defer rows.Close()
...
func handleAdminSuspendWorkspace(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminSuspendWorkspace", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.JSON(fiber.Map{"success": true})
}
...
func handleAdminReactivateWorkspace(c *fiber.Ctx) error {
	...
	if err != nil {
		slog.Error("handleAdminReactivateWorkspace", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	return c.JSON(fiber.Map{"success": true})
}
...
func handleAdminListAuditLog(c *fiber.Ctx) error {
	...
	rows, err := db.Pool.Query(ctx,
		`SELECT a.id, a.action, a.entity_type, a.entity_id, a.old_value, a.new_value,
		 ...
		 LIMIT 100`)
	if err != nil {
		slog.Error("handleAdminListAuditLog", "err", err)
		return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
	}
	defer rows.Close()
...
func handlePublicOptions(table, orderCol string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			slog.Error("handlePublicOptions", "table", table, "err", err)
			return fiber.NewError(fiber.StatusInternalServerError, "erro interno do servidor")
		}
		defer rows.Close()
