import { test, expect } from './fixtures/auth';

test('cards-invoices: criar cartao + ver fatura', async ({ authedPage: page }) => {
  await page.goto('/cards');
  await page.getByTestId('btn-new-card').click();
  await page.getByTestId('input-card-name').fill('Cartao E2E');
  // Banco via shadcn Select: click trigger + escolher option
  await page.getByTestId('select-card-bank').click();
  await page.getByRole('option', { name: 'Nubank' }).click();
  await page.getByTestId('input-card-limit').fill('5000');
  await page.getByTestId('input-card-closing-day').fill('5');
  await page.getByTestId('input-card-due-day').fill('15');
  await page.getByTestId('btn-save-card').click();
  await expect(page.getByText('Cartao E2E')).toBeVisible({ timeout: 10_000 });
  await page.goto('/invoices');
  await expect(page.getByTestId('list-invoices')).toBeVisible();
});
