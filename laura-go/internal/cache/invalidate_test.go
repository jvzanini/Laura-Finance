...
func TestInMemoryCache_InvalidateWorkspace_AllScopes(t *testing.T) {
	...
	_ = c.Set(ctx,"ws:abc:dashboard:x", []byte("1"), time.Minute)
	_ = c.Set(ctx,"ws:abc:score:y", []byte("2"), time.Minute)
	_ = c.Set(ctx,"ws:def:dashboard:z", []byte("3"), time.Minute)
...
func TestInMemoryCache_InvalidateWorkspace_SpecificScopes(t *testing.T) {
	...
	_ = c.Set(ctx,"ws:abc:dashboard:x", []byte("1"), time.Minute)
	_ = c.Set(ctx,"ws:abc:score:y", []byte("2"), time.Minute)
	_ = c.Set(ctx,"ws:abc:reports:z", []byte("3"), time.Minute)
