# Fase 16 — Infra Hardening (spec v2 — review #1)

**Mudanças vs v1:**
- Confirmado `system_config` existe (migration 000025).
- Migration drill: cron semanal + manual dispatch (não por push).
- Coverage merge: via `gocovmerge` instalado no CI.
- Plano B whatsmeow: se mock invasivo, apenas cobre código "pure"
  (env parsing, disable flag, timing helpers) e aceita 10% em vez
  de 20%.
